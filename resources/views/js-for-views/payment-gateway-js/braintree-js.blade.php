...
<script>
    var PRODUCT_TYPE = "{{ $productItem->type }}";
    var PRODUCT_ID = "{{ $productItem->id }}";
    var basicFormBt;
    ...
    function validateData() {
        $.ajax({
            url: "{{ url('checkout/validate') }}" + "/" + "{{$productItem->id}}" + "/" + "{{$productItem->slug}}",
            method: "POST",
            data: basicFormBt,
            processData: false,
            contentType: false,
    ...
    btInstance.requestPaymentMethod({
        ...
        document.querySelector('#course_bt').value = "{{$productItem->id}}";
